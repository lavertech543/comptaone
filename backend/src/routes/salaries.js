...
router.post('/employees', requirePermission(M,'create'), async (req, res) => {
  const { nom, poste, salaire_ref, date_entree, statut } = req.body;
  if (!nom) return res.status(400).json({ error: 'Nom requis' });
  const r = await query('INSERT INTO employees(nom,poste,salaire_ref,date_entree,statut) VALUES($1,$2,$3,$4,$5) RETURNING id',
    [nom, poste, salaire_ref||0, date_entree||null, statut||'actif']);
...
router.put('/employees/:id', requirePermission(M,'edit'), async (req, res) => {
  const { poste, salaire_ref, statut } = req.body;
  await query('UPDATE employees SET poste=COALESCE($1,poste),salaire_ref=COALESCE($2,salaire_ref),statut=COALESCE($3,statut) WHERE id=$4',
    [poste, salaire_ref, statut, req.params.id]);
